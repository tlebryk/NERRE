...
def evaluate(gold, test, loud=False, lowercase=False):
    ...
    scores = {k: {k2: 0 for k2 in ["tp", "tn", "fp", "fn"]} for k in ent_categories}

    scores["dopants2basemats"] = {
        "n_correct": 0,
        "test_retrieved": 0,
        "gold_retrieved": 0,
    }
    ...
    for i, val_entry in enumerate(gold):
        for j, s in enumerate(val_entry["doping_sentences"]):
            ...
            test_entry = {
                k: test_entry_tot[k] for k in ent_categories + ["dopants2basemats"]
            }
            ...
            if lowercase:
                ...
                gold_completion = json.dumps(
                    {
                        k: gold_entry[k]
                        for k in ["dopants", "basemats", "dopants2basemats"]
                    }
                )
                ...
                try:
                    test_completion = test[i]["doping_sentences"][j]["llm_completion"]
                except KeyError:
                    print(
            ...
            for ent_type in ent_categories:
                ...
                if ent_type == "doping_modifiers":
                    ...
                    gold_ents_words = " ".join(
                        list(gold_entry[ent_type].values())
                    ).split(" ")

                support["words"][ent_type] += len(gold_ents_words)
                support["ents"][ent_type] += (
                    1 if isinstance(gold_ents, str) else len(gold_ents)
                )
                        ...
                        for bmat_word in bmat_words.split(" "):
                            for dop_word in dop_words.split(" "):
                                if bmat_word and dop_word:
                                    rel_entry["triplets"].append(
                                        f"{bmat_word} {dop_word}"
                                    )
    ...
    if triplet_recall == 0 or triplet_prec == 0:
        ...
        triplet_f1 = (2 * triplet_prec * triplet_recall) / (
            triplet_prec + triplet_recall
        )
    print(f"triplets: prec={triplet_prec}, recall={triplet_recall}, f1={triplet_f1}")
    scores_computed["link triplets"] = {
...
def main(test_file, pred_file, plot, loud, schema_type, enforce_lowercase, output_dir):
    ...
    for gj in gold:
        for sjson in gj["doping_sentences"]:
            c = llm_completion_from_sentence_json(
                sjson, stop_token="", fmt=fmt, **kwargs
            )
            sjson["completion"] = c
